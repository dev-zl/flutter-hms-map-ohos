...
abstract class _HuaweiMapMethodChannel {
  ...
  static String _getChannelPrefix() {
  ...
  static Future<void> init(int mapId) async {
    if (!_channels.containsKey(mapId)) {
      final String channelPrefix = _getChannelPrefix();
      final MethodChannel channel = MethodChannel('${channelPrefix}_$mapId');
      debugPrint(
          'HuaweiMap init: creating MethodChannel ${channelPrefix}_$mapId');
      ...
      debugPrint(
          'HuaweiMap init: invoking ${_Method.MapWaitForMap} mapId=$mapId');
      await channel.invokeMethod<void>(
        _Method.MapWaitForMap,
      );
      debugPrint(
          'HuaweiMap init: ${_Method.MapWaitForMap} completed mapId=$mapId');
    }
  }
    ...
    if (PlatformDetector.isHarmonyOS) {
      ...
      return LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          debugPrint(
            'HuaweiMap init: OHOS layout constraints '
      ...
      debugPrint(
        'HuaweiMap init: building Android PlatformViewLink viewType=$_mapChannel '
      ...
      return PlatformViewLink(
        ...
        onCreatePlatformView: (PlatformViewCreationParams params) {
          debugPrint(
            'HuaweiMap init: initExpensiveAndroidView id=${params.id} '
            'viewType=$_mapChannel',
          );
